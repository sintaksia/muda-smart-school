...
export function FaqActions({ faq }: FaqActionsProps) {
  ...
  const handleDelete = async () => {
    setIsLoading(true);
    try {
      const response = await fetch(`/api/cms/faqs/${faq.id}`, {
        method: "DELETE",
      });
  ...
  const handleToggleStatus = async () => {
    try {
      ...
      toast.success(faq.isActive ? "FAQ dinonaktifkan" : "FAQ diaktifkan");
      router.refresh();
    } catch {
  ...
  return (
    <>
      ...
      <DeleteDialog
        ...
        isLoading={isLoading}
        title="Hapus FAQ"
        description={`Apakah Anda yakin ingin menghapus FAQ "${faq.question}"?`}
      />
    </>
  );
}
